...
class ArrayUtil
{
     ...
     * ```
     *
     * The result of `ArrayUtil::toArray($post, $properties)` could be like the following:
     *
     * ```php
     * [
     *     'id' => 123,
     ...
     * $removed = ArrayUtil::removeValue($array, 'Jackson');
     * // result:
     ...
     *
     * ```php
     * $result = ArrayUtil::index($array, function ($element) {
     *     return $element['id'];
     * });
     * ```
     *
     ...
     *
     * ```php
     * $result = ArrayUtil::index($array, null, 'id');
     * ```
     *
     ...
     *
     * ```php
     * $result = ArrayUtil::index($array, 'data', [function ($element) {
     *     return $element['id'];
     * }, 'device']);
     * ```
     *
     ...
     * $result = ArrayUtil::getColumn($array, function ($element) {
     *     return $element['id'];
     * });
     * ```
     *
     ...
     * ];
     *
     * $result = ArrayUtil::filter($array, ['A']);
     * // $result will be:
     * // [
     * //     'A' => [1, 2],
     * // ]
     *
     * $result = ArrayUtil::filter($array, ['A', 'B.C']);
     * // $result will be:
     * // [
     * //     'A' => [1, 2],
     * //     'B' => ['C' => 1],
     * // ]
     *
     * $result = ArrayUtil::filter($array, ['B', '!B.C']);
     * // $result will be:
     * // [
     * //     'B' => ['D' => 2],
     * // ]
     ...
     * @return array
     */
    public static function childKeepers($array, $keepers)
    {
//        ['a' => 'ddd', 'b'=>11],
        is_string($keepers) && $keepers = explode(',', $keepers);
        $result = [];
        foreach ($array as $k => $v) {
            $item = [];
            foreach ($keepers as $keeper) {
                $keeper = trim($keeper);
                $item[$keeper] = $v[$keeper];
            }
            $result[$k] = $item;
        }
     ...
     * @param $where array 条件['key1' => 'val1', 'key2' => 'val2', ['like/!=/not in', 'field', 'value']……]
     * @param $keep_key bool 保持key
     * @return array 满足条件的值组成的数组
     */
    public static function getWhereData($array, $where, $keep_key = true)
    {
        $arr = [];
        if (!is_array($array)) {
            return [];
        ...
        foreach ($array as $id => $one) {
            ...
            if (reset($where) === 'and') {
                ...
                foreach ($where as $wh) {
                    ...
                    $array = self::getWhereData($array, $wh, $keep_key);
                }

                return $array;
            } //or条件查询, ['or', $where1, $where2, ……]
            elseif (reset($where) === 'or') {
                unset($where[0]);
                $arr = [];
                foreach ($where as $wh) {
                    $result = self::getWhereData($array, $wh);
            ...
            elseif (in_array(reset($where), ['!=', '<=', '<', '>=', '>', 'like', 'not in'], true)) {
                list($operate, $field, $value) = $where;
                switch ($operate) {
                    case '!='://不等于
                        if (!($one[$field] != $value)) {
                            $flg = false;
            ...
            elseif (in_array(reset($where), ['between'], true)) {
                list($operate, $field, $value1, $value2) = $where;
                switch ($operate) {
